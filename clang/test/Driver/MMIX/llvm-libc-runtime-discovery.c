...
// RUN:   | FileCheck %s --check-prefix=NOT-COMPOSED --implicit-check-not=ld.lld
...
// RUN:   | FileCheck %s --check-prefix=NOT-COMPOSED \
// RUN:       --implicit-check-not='no such file or directory' \
// RUN:       --implicit-check-not=ld.lld
// RUN: not %clang -### --target=mmix-unknown-elf --cstdlib=llvm-libc \
// RUN:   --sysroot=%t.dir/sysroot -resource-dir=%t.dir/resource \
// RUN:   %s -o %t.dir/output 2>&1 \
// RUN:   | FileCheck %s --check-prefix=NOT-COMPOSED --implicit-check-not=ld.lld
...
// RUN:   | FileCheck %s --check-prefix=NOT-COMPOSED \
// RUN:       --implicit-check-not=crt1.o --implicit-check-not=ld.lld
// RUN: touch %t.dir/sysroot/lib/mmix-unknown-unknown/crt1.o
...
// RUN:   | FileCheck %s --check-prefix=NOT-COMPOSED \
// RUN:       --implicit-check-not=mmix-qemu.ld --implicit-check-not=ld.lld
...
// RUN:   | FileCheck %s --check-prefix=NOT-COMPOSED \
// RUN:       --implicit-check-not=libc.a --implicit-check-not=ld.lld
...
// RUN:   | FileCheck %s --check-prefix=NOT-COMPOSED \
// RUN:       --implicit-check-not='{{[/\\]libm\.a}}' --implicit-check-not=ld.lld
...
// NOT-COMPOSED: error: the clang compiler does not support 'LLVM libc link composition for MMIX'
