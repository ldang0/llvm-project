# RUN: not llvm-mc -triple=mmix -filetype=obj %s -o %t 2>&1 | FileCheck %s
# RUN: not test -e %t

branch_misaligned:
BN r1, branch_misaligned_target
.set branch_misaligned_target, branch_misaligned + 2
# CHECK: error: MMIX 19-bit terminal target is not instruction aligned

branch_forward_direction:
BN r1, branch_before
.set branch_before, branch_forward_direction - 4
# CHECK: error: MMIX 19-bit terminal target direction does not match instruction
...
# CHECK: error: MMIX 19-bit terminal target is out of range
...
# CHECK: error: MMIX 27-bit terminal target is not instruction aligned

jump_forward_direction:
JMP jump_before
.set jump_before, jump_forward_direction - 4
# CHECK: error: MMIX 27-bit terminal target direction does not match instruction
...
# CHECK: error: MMIX 27-bit terminal target is out of range
